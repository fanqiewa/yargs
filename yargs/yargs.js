...
function Yargs (processArgs, cwd, parentRequire) {
  ...
  // process.cwd() 方法会返回 Node.js进程的当前工作目录。
  ...
  /**
   * 将键解析为布尔标志，但将其解析值设置为标志出现的次数，而不是true或false
   * 因此默认值为0
   * @param {Array|String} keys keys
   * e.g. 
   * .count("count")
  ...
  /**
   * 将键的有效值限制为预定义的一组选项，以数组或单个值得形式给出。
   * @param {Array|String|Array} keys keys
   * e.g. 
   * .choices("name", ["fanqiewa", "yiran"]);
   */
   ...
   * @param {Array|String|Array} key key
   * @param {*} value value
   * @param {String} defaultDescription 描述
   * e.g. 
   * .default("random", function randomValue() { return Math.random() * 256; });
   */
   ...
   * 通过同步的函数来转换命令行上给出的值
   * @param {Array|String|Array} keys keys
   * @param {String|Array} value value 可选
   * e.g. 
   ...
   * 该API已被废弃
   * 设置必须传入的值
   * @param {Number|Array|String} keys keys
   * @param {String|Array} max 可选
   * @param {String} msg msg 可选
   * e.g. 
   * require("./yargs")(['run', 'path', '--run', 'serve', '--path', '/index.js']);
   * .demand(2, ['run', 'path'], 'Please provide both run and path arguments to work with this tool');
   * 
   * require("./yargs")(['--run', 'serve', '--path', '/index.js']);
   * .demand('run', 'Please provide both run and path arguments to work with this tool');
   */
  ...
  /**
   * 设置冲突的键（即：key1和key2不能同时存在）
   * @param {String|Object} key1 key1
   * @param {String|Array} key2 key2 可选
   * e.g. 
   * require("./yargs")(['--x', 'yiran', '--z', 'fanqiewa']);
   * .conflicts('x', ['y', 'z']);
   * -- 输出报错信息，因为x和z同时存在
   * 
   * .conflicts('x', 'y');
   ...
   * @param {Boolean} _global 可选参数，默认为false
   * e.g. 
   * .check((argv, options) => { }, false);
   */
